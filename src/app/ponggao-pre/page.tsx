...
export default function OrderBeta() {
  ...
  const getOrder = async () => {
    const res = await axios.get(HOST + '/api/order/order-pre')
    const list =
    ...
    <div>
      <div>
        ...
        {orderViews.map((order, index) => (<div className='mb-6 ' key={order._id}>
          <div>STT: {order.stt} <span className="text-center text-lg text-white font-semibold">{order.products.filter(p => p.pre).length ? '[PRE-ORDER] ' : ' '}{order.shipOption=='all' ? 'Đợi gộp' : 'Tách đơn, ship đồ có sẵn trc, preorder sau'}</span></div>
